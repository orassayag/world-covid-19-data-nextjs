...
const MasterActions = memo(
  ({ isActive, isLiveMode, isRecoverMode, onActionClick }) => {
    ...
    const actionValues = [
      ...
      isLiveMode ? 'off' : 'on',
      isLiveMode ? 'on' : 'off',
      isRecoverMode ? 'on' : 'off',
      null,
    ];
    const actionIconAttributeNames = [
      'active',
      'refresh',
      'statistics',
      'local',
    ...
    const actionIconTooltips = [
      `Active (${isActive ? 'On' : 'Off'})`,
      'Refresh data',
      'Statistics',
      'Local data mode',
      'Live data mode',
      `Recover data mode (${isRecoverMode ? 'On' : 'Off'})`,
      'Credits',
    ];
    ...
    for (let i = 0; i < creditService.masterCredits.length; i += 1) {
      const { title, link, iconName, iconType, iconTooltip } =
    ...
    return (
      <div className={styles.actions}>
        ...
        {iconsDOM.slice(3, 6)}
        <div className={styles.contact}>Help</div>
        {iconsDOM.slice(6, 10)}
      </div>
    );
  }
