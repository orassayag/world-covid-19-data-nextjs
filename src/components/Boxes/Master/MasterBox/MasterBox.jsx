...
const MasterBox = memo(({ onActionClick }) => {
  ...
  const statisticsList = useSelector(
    (state) => state.statistics.statisticsList
  );
  const {
    isActive,
    isLiveMode,
  ...
  const {
    ...
    isLastUpdateChanges,
    totalVisibleCountriesCount,
  } = statisticsList;
  const currentTime = useSelector((state) => state.statistics.currentTime);
  const isUpdateLoaderDisplay = useSelector(
    (state) => state.statistics.isUpdateLoaderDisplay
  );
  const lastUpdateDateDisplay = useSelector(
    (state) => state.statistics.lastUpdateDateDisplay
  );

  return (
    <div className={`${styles.box} ${styles.master}`}>
      {isActionLoader && (
        <MasterActionLoader refreshSourceName={loadingList.refreshSourceName} />
      )}
      {!isActionLoader && (
