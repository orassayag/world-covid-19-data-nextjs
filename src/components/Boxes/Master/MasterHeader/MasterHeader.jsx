...
const MasterHeader = memo(
  ({
    ...
    const valuesClasses = [
      '',
      '',
      ` source${leadingSource.upperName === lastUpdateSourceName && isLastUpdateChanges ? ' alert' : ''}`,
    ];
    const itemsDOM = [];
    for (let i = 0; i < 3; i += 1) {
      itemsDOM.push(
    ...
    return (
      <div className={styles.logo_holder}>
        <div className={styles.line1}>
          <h3>World Covid</h3>
          <div className={styles.items}>{itemsDOM}</div>
          <MasterBinaryClock currentTime={currentTime} />
        </div>
        <div className={styles.line2}>
          <p>
            ...
            >{` | ${dataModDisplay} mode | ${intervalSeconds}sec`}</span>
          </p>
        </div>
      </div>
    );
