...
export default function CountrySource({ source, data, updateSourceName }) {
  ...
  if (data) {
    ...
    for (let i = 0; i < dataItems.length; i += 1) {
      itemsDOM.push(<CountrySourceItem key={i} dataItem={dataItems[i]} />);
    }
    itemsDOM = <div className={styles.items}>{itemsDOM}</div>;
  } else {
  ...
  return (
    <div
      className={`${styles.source} ${!isActive || isError ? styles.not_active : ''}`}
    >
      <CountrySourceName
        upperName={upperName}
        lowerName={lowerName}
