...
export default function CountryData({
  ...
  let dataDOM = null;
  if (isContainData) {
    const updateSourceName = updateSourceData
      ? updateSourceData.sourceName
      : '';
    dataDOM = (
      <>
        <div className={styles.live_data}>
          <CountryDataTitle />
          <CountryUpdatePanel updateSourceData={updateSourceData} />
        ...
        />
        <CountrySummary sourcesList={sourcesList} summaryData={summaryData} />
      </>
    );
  } else {
  ...
  return (
    <div
      className={`${styles.country_data} ${isContainData ? '' : styles.no_data}`}
    >
      {dataDOM}
      <CountryOkButton onActionClick={onActionClick} />
    </div>
  );
}
