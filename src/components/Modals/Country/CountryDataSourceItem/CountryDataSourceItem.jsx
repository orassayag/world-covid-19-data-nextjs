...
export default function CountryDataSourceItem({
  ...
  if (data) {
    ...
    itemsDOM.push(<CountryDataSourceDisplayItemNoData key={lowerName} />);
  }

  return (
    <div
      className={`${styles.data_item} ${!isActive || isError ? styles.not_active : isAlert ? styles.alert : ''}`}
    >
      <div className={`${styles.name} ${isAlert ? styles.active : ''}`}>
        {upperName}
      </div>
