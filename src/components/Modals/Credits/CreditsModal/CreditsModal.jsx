...
export default function CreditsModal({ styles, onActionClick }) {
  const { created, creator, years, tooltip } =
  ...
  return (
    <div className={`${styles.modal_content} ${styles.credits} f16 f16-extra`}>
      <div className={styles.content}>
        <CreditsExitButton styles={styles} onActionClick={onActionClick} />
        <div className={localStyles.credits}>
          <div className={localStyles.title}>Credits</div>
          <div className={localStyles.data}>{creditsLinksDOM}</div>
          <div className={localStyles.note}>
            <span>Please note:</span> The project is not yet finished, and not
            all functionality works. Feel free to{' '}
            <a href='mailto:orassayag@gmail.com'>contact me</a> for any
            mistakes, bugs or have any comments or ideas to share.
          </div>
          <div className={localStyles.creator}>
            {created}{' '}
            <a
              href={creditService.allCredits[3].link}
        ...
        <CreditsOkButton styles={styles} onActionClick={onActionClick} />
      </div>
    </div>
  );
}
